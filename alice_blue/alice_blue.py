...
class AliceBlue:
    ...
    def __init__(self, username, password, access_token, master_contracts_to_download=None):
        ...
        if (profile['status'] == 'error'):
            if (profile[
                'message'] == 'Not able to retrieve AccountInfoService'):  # Don't know why this error comes, but it safe to proceed further.
    ...
    @staticmethod
    def login_and_get_access_token(username, password, twoFA, api_secret,
                                   redirect_url='https://ant.aliceblueonline.com/plugin/callback', app_id=None):
        ...
        resp = r.post(resp.url, data={'client_id': username, 'password': password, 'login_challenge': login_challenge,
        ...
        resp = r.post(resp.url, data={'answer1': twoFA, 'question_id1': question_ids, 'answer2': twoFA,
                                      'login_challenge': login_challenge, '_csrf_token': csrf_token})
        if ('consent_challenge' in resp.url):
            ...
            if ('Internal server error' in resp.text):
                logger.error(
                    f"Getting 'Internal server error' while authorizing the app for the first time. Please login manually using the following url '{url}'")
                return
        code = resp.url[resp.url.index('=') + 1:resp.url.index('&')]

        # Get Access Token
        params = {'code': code, 'redirect_uri': redirect_url, 'grant_type': 'authorization_code',
                  'client_secret': api_secret, "cliend_id": username}
    ...
    def __ws_run_forever(self):
        while True:
            try:
                ...
                pass  # logger.warning(f"websocket run forever ended in exception, {e}")
